...
export const GROUP_MATCHES = RAW.map((r, i) => ({
  id: "g" + (i + 1), stage: "group", group: r[0], t1: r[1], t2: r[2], ko: r[3] + ":00-04:00",
}));
...
export function buildDates(matches) {
  const set = new Set();
  let hasElim = false;
  matches.forEach(m => { const d = dateKeyOf(m.ko); if (d) set.add(d); else if (m.stage === "ko") hasElim = true; });
  const arr = [...set].sort();
...
export function fmtKO(ko) {
  ...
  return `${DOW[new Date(+y, +mo - 1, +d).getDay()]} ${+d} ${MON[+mo - 1]} · ${hh}:${mm} ET`;
}

export const isLocked = (ko) => { if (!ko) return false; const t = Date.parse(ko); return !Number.isNaN(t) && Date.now() >= t; };
...
export const isBeforeWindow = (ko) => {
  if (!ko) return false;
